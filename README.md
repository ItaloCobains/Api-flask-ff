...
- 🎯 Entry points to execute your program using `python -m <api_flask_ff>` or `$ api_flask_ff` with basic CLI argument parsing.
...
---
# api_flask_ff Flask Application

Awesome api_flask_ff created by ItaloCobains
...
```bash
git clone https://github.com/ItaloCobains/Api-flask-ff api_flask_ff
cd api_flask_ff
make install
```
...
```bash
api_flask_ff create-db   # run once
api_flask_ff populate-db  # run once (optional)
api_flask_ff add-user -u admin -p 1234  # ads a user
api_flask_ff run
```
